// Feb 2022
// For details, see wsurvey.canvasChart.txt.
...
// Usage:
//      Include something like: <script src="wsurveyCanvasChart.js" type="text/javascript"></script>
// and loading jQuery with something like
//
// Call using: <script  src="jQuery-3.6.0.min.js" type="text/javascript"></script>
...
wsurvey.canvasChart = function () {
    ...
    var renderType = { lines: 'lines', points: 'points',textValues:'textValues',bars:'bars',line:'lines',point:'points',text:'textValues',bar:'bars'}; // note use of synonyms
...
// ... this is the public facing function. Sets locally global variables, etc.
   ...
   // comment=(dataObj.hasOwnProperty('comment')) ? dataObj['comment'] : 'noComment';
       ...
       yPrec=0;xPrec=0; yGridLinesPx=30;yGridLinesColor='#E8E8E8'; yAxisRight=0;
       refLinesY=[];refLinesX=[];
       ...
       if (data.hasOwnProperty('chartSpecs')  ) {                          // extract and validate chartSpecs
        ...
        if (typeof(data.xPrec)!=='undefined') xPrec=data.xPrec;
...
// add shapes to the "hitImage" -- that may have prior shapes on it already.
     ...
     if (debugMode>1) {
         ...
         if (oy.length==1) {
             let oymess='<ul>'
             let     Stamp = new Date(chartSpecs.hitInfo.retTime);
              let atime=Stamp.toString();
...
/// add a mousedown /mouse up click handler?
     if (clickFunc!=='' && noClickfunc==0 && createHitInfo==1)  {       // click handler desired AND event handler exists  AND click info update not suppressed
        ...
        let evtData2=JSON.parse(JSON.stringify(evtData)); // this ensures that the clicks work EVEN if there are differet images drawn
         evtData2['clickFunc']= clickFunc ;  // the function reference (not a string with its name)
         evtData2['clickFuncPriorLoc']={};
//         evtData2['clickFuncNowLoc']={};  // feb 2022 deprecated
...
//     wsurvey.dumpObj(defMargins,1,'ddfff2');

     let keyLeft=defMargins['left'],keyTop=defMargins['top'],keyRight=defMargins['right'],keyBottom=defMargins['bottom'];  // easir to work with scalars...
...
// selection test here.
           ...
           if (xCategory==1) {
            ...
            if (gotFirst==1 &&  (type == renderType.lines || type==='lines') ) {        //  Draw connecting lines    (note if i=0, save for use in i=1 (start of line)    .........
...
// barsDone :  dataPoint id , upper left x ploc, upper left y ploc, width, height
            ...
            if (type == renderType.points || type==='points') {         // :::::::::::::  draw a SHAPE  (a circle,square,triangl: outline, filled, gradient circle; or a + or X  ........
             ...
             if (type == renderType.bars || type==='bars') {                                   // draw a histogram bar ....
               ...
               ctx.globalCompositeOperation=gCompositeOperation;
                ...
                let barWidePxUse = (isNaN(barWidePx)) ? dataPointSize : Math.max(parseFloat(dataPointSize),4) ;
//                let cool1= (!isNaN(barWidePx)) ? parseFloat(ptX)+parseFloat(barWidePx/3) : parseFloat(ptX) ;
                let cool1= parseFloat(ptX+(barWidePxUse/3)) ;  // in pixel space ... (ptX and pxY in pixel, ptXval and ptyVal are origional values)
                if (i==0) {     // if first bar, center it and use half bar. Even if barSide != 'A'
                      x0=ptX;
                      barWidePxUse=barWidePxUse/2;
                } else if (barSide=='S') {
                       x0=ptX ;
                } else if (barSide=='E') {  // bar right edge over x0
                       x0=ptX-barWidePxUse ;
                } else {   //   defaiult is centered
                     x0=ptX-(barWidePxUse/2);
                }
                ...
                ctx.fillStyle = lingrad;
                ctx.fillRect(x0,ptY,barWidePxUse,aheight );
                apt=[i,x0-margin.left,ptY-margin.top,barWidePxUse/3,aheight] ;   // barid, upper left corner x and y, width,height
                barsDone.push(apt) ;
            }

            if (type == renderType.textValues || type==='text') {             // display Y value  (or label value) next to point   ..............
                  ctx.globalCompositeOperation=gCompositeOperation;
                  ctx.beginPath();
...
//====================
// plot xy graphs, y can be more than one variable (so more than one graph)
// this is a front end to render
var plotXY=function(canvasId,x0,y0,voptions) {

  if (arguments.length<4) voptions={};

 let x=JSON.parse(JSON.stringify(x0));
 if (!Array.isArray(x)) return false ;
 let y=JSON.parse(JSON.stringify(y0));
 if (!Array.isArray(y)) return false ;

  let minY=100000000000000000,maxY=-1000000000000000000;
  let minX=100000000000000000,maxX=-1000000000000000000;
...
// convert y values to same size arrays .. and compuute min max of x and y
 let nY=0;
 for (let jy=0;jy<y.length;jy++) {
     if (jy>=x.length) continue ;        // skip if no X
     if (!Array.isArray(y[jy])) y[jy]=[y[jy]];
     nY=Math.max(nY,y[jy].length);
 }
 for (let jy=0;jy<y.length;jy++) {      // all  elememts should be arrays of same length;... pad with '' if necessary
    if (jy>=x.length) continue ;        // skip if no X
    for (let iy2=0;iy2<y[jy].length;iy2++) {
        let ay=y[jy][iy2] ;
    ...
    if (y[jy].length<nY) {         // fill
         for (let mm=y[jy].length;mm<nY;mm++) y[jy][mm]='';
    }
 }

//  general specs....

  let dspecs={},dstuff=[],iz=0;;

  let otitle=(voptions.hasOwnProperty('title')) ? voptions['title'] : 'Sample graph ';
  let xtitle=(voptions.hasOwnProperty('xTitle')) ? voptions['xTitle'] : 'X ';
  let ytitle=(voptions.hasOwnProperty('yTitle')) ? voptions['yTitle'] : 'Y ';
  let keytitle=(voptions.hasOwnProperty('key')) ? voptions['key'] : 'Key';
  let dataPointSize=(voptions.hasOwnProperty('dataPointSize')) ? voptions['dataPointSize'] : 4 ;
  let lineWidth=(voptions.hasOwnProperty('lineWidth')) ? voptions['lineWidth'] : 2 ;

  let refLinesY=(voptions.hasOwnProperty('refLinesY')) ? voptions['refLinesY'] : '0.0 gray dashed' ;
  let refLinesX=(voptions.hasOwnProperty('refLinesX')) ? voptions['refLinesX'] : '0.0 gray dashed' ;

  let titleFont=(voptions.hasOwnProperty('titleFont')) ? voptions['titleFont'] : '12pt arial' ;
  let axisTicFont=(voptions.hasOwnProperty('axisTicFont')) ? voptions['axisTicFont'] : '8pt arial' ;

  let chartMargin=(voptions.hasOwnProperty('chartMargin')) ? voptions['chartMargin'] : {'top':40,'left':115,'bottom':75,'right':150} ;

  let skipYout=(voptions.hasOwnProperty('skipYout')) ? voptions['skipYout'] : 1  ;

// color specs
  let colors=['green','blue','red','cyan','gold'];
  if (voptions.hasOwnProperty('colors')) {
      let dcolors=voptions['colors'] ;
      if (Array.isArray(dcolors))   {     // legit spec of colors
         for (let ic=0;ic<dcolors.length;ic++) {
              acolor=jQuery.trim(dcolors[ic]);
              colors[ic]=acolor;
         }
     }
  }
  if (colors.length<nY)   {         // not enough colors -- use brown
    for (let ic0=colors.length;ic0<nY;ic0++)   colors[ic0]='brown';
  }

// symbol specs
  let symbols=['C','S','T','D','X'];   // circle square triagnle diamond X
  if (voptions.hasOwnProperty('symbols')) {
      let dsymbols=voptions['symbols'] ;
      if (Array.isArray(dsymbols))   {     // legit spec of colors
         for (let ic=0;ic<dsymbols.length;ic++) {
              asymbol=jQuery.trim(dsymbols[ic]);
              symbols[ic]=asymbol;
         }
     }
  }
  if (symbols.length<nY)   {         // not enough colors -- use brown
    for (let ic0=symbols.length;ic0<nY;ic0++)   symbols[ic0]='+';
  }

// dashes specs
  let dashes=['0','0','0','0','0'];   // circle square triagnle diamond X
  if (voptions.hasOwnProperty('dashes')) {
      let ddashes=voptions['dashes'] ;
      if (Array.isArray(ddashes))   {     // legit spec of colors
         for (let ic=0;ic<ddashes.length;ic++) {
              adash=jQuery.trim(ddashes[ic]);
              dashes[ic]=adash;
         }
     }
  }
  if (dashes.length<nY)   {         // not enough colors -- use brown
    for (let ic0=dashes.length;ic0<nY;ic0++)   ddashes[ic0]='0';
  }


// names
  let vnames=[];
  for (let inn=0;inn<nY;inn++)
     vnames[inn]='Y'+(1+inn);
    if (voptions.hasOwnProperty('names')) {
      let dnames=voptions['names'] ;
      if (Array.isArray(dnames))   {     // legit spec of colors
         for (let ic=0;ic<dnames.length;ic++) {
              aname=jQuery.trim(dnames[ic]);
              vnames[ic]=aname;
         }
     }
  }
...
// plot each y!
  for (let iz=0;iz<nY;iz++) {
    ...
    let  dstuff=[];               // seperate datapoints for each x,y plot ...
    for (let jj=0;jj<x.length;jj++) {   // create datapoints to be plotted  -- for each X (ignore y rows if no x row
      ...
      let tt={'x':ax,'y':ay,'ID':jj }  ;
      dstuff.push(tt);
    }            //  jj ... datapoints ready to plot!

    dspecs['dataPoints']=dstuff;

    let xrangeUse,yrangeUse;
   if (voptions.hasOwnProperty('xrange')) {
       xrangeUse=voptions['xrange'] ;
    } else {
      xrangeUse=[minX,maxX,6].join(',');
    }
   if (voptions.hasOwnProperty('yrange')) {
       yrangeUse=voptions['yrange'] ;
    } else {
        yrangeUse=[minY,maxY,6].join(',')
    }
    ...
    if (iz==0) {                          // do these on first graph
        ...
        dspecs['title']=otitle;
        dspecs['xLabel']=xtitle ;
        dspecs['yLabel']=ytitle;
        dspecs['refLinesY']= refLinesY;
        dspecs['refLinesX']=refLinesX ;

        dspecs['titleFont']=titleFont ;
        dspecs['axisTicFont']=axisTicFont;
      ...
      let zcolor=colors[iz],qline=false;
      if (zcolor!=='') {
         dspecs['lineColor']=colors[iz];
         dspecs['lineWidth']=lineWidth;
         dspecs['lineDotted']=dashes[iz];
         qline=true;
      }

      let zsymbol=symbols[iz],qsymbol=false;
      if (zsymbol!=='') {
        dspecs['dataPointType']=symbols[iz];
        dspecs['dataPointColor']=colors[iz] ;
        dspecs['dataPointSize']=dataPointSize;
        qsymbol=true;
      }

      dspecs['xRange']=xrangeUse ;
      ...
      let rtypes=[];
      if (qsymbol) rtypes.push('points');
      if (qline) rtypes.push('lines');
      dspecs['renderTypes']=rtypes ;
      let arf=wsurvey.canvasChart.render(canvasId,dspecs);  // for this y
   }         // iz
...
// used in the key
  ...
  if (keytitle=='') return 1  ; // no key
  let datexts=[],dasymbols=[];
  for (let ij=0;ij<nY;ij++) {
      let aa=vnames[ij]   ;
      datexts.push(aa);
